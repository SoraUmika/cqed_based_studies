...
def build_square_multitone_compiled(
    model: DispersiveTransmonCavityModel,
    spec: TargetSpec,
    run_config: ConditionedMultitoneRunConfig,
    *,
    ...
    return waveform, tone_specs, compiled
...
def embed_restricted_operator_in_full(
    ...
    indices = np.asarray(logical_indices(model, levels), dtype=int)
    full[np.ix_(indices, indices)] = np.asarray(restricted_operator, dtype=np.complex128)
    return full
...
def scaled_target_spec(
    ...
    *,
    ...
    return TargetSpec(
        family=f"{spec.family}{family_suffix}",
        theta_values=tuple(float(theta_scale * value) for value in spec.theta_values),
        phi_values=tuple(float(phi_sign * value) for value in spec.phi_values),
        metadata={
            **dict(spec.metadata),
            "family": f"{spec.family}{family_suffix}",
            ...
            "description": (
                ...
                else f"Scaled target derived from {spec.family} with theta_scale={theta_scale} and phi_sign={phi_sign}"
            ),
            "theta_values_rad": [float(theta_scale * value) for value in spec.theta_values],
            "phi_values_rad": [float(phi_sign * value) for value in spec.phi_values],
        },
    )
...
def logical_state_qobj_from_vector(
    model: DispersiveTransmonCavityModel,
    levels: Sequence[int],
    logical_vector: Sequence[complex],
) -> qt.Qobj:
    vec = np.asarray(logical_vector, dtype=np.complex128).reshape(-1)
    if vec.size != 2 * len(levels):
        raise ValueError(f"Expected logical vector of length {2 * len(levels)}, received {vec.size}.")
...
def probe_state_definitions(levels: Sequence[int]) -> tuple[dict[str, Any], ...]:
    n_levels = len(levels)
    equal_cavity = np.ones(n_levels, dtype=np.complex128) / np.sqrt(n_levels)
    alt_cavity = np.asarray([1.0 if idx % 2 == 0 else -1.0 for idx in range(n_levels)], dtype=np.complex128) / np.sqrt(n_levels)
    logical_plus = np.asarray([1.0, 1.0], dtype=np.complex128) / np.sqrt(2.0)
    logical_plus_i = np.asarray([1.0, 1.0j], dtype=np.complex128) / np.sqrt(2.0)
    rows: list[dict[str, Any]] = [
        {"label": "equal_ground", "kind": "cavity_x_qubit", "cavity": equal_cavity, "qubit": np.asarray([1.0, 0.0], dtype=np.complex128)},
        {"label": "equal_plus", "kind": "cavity_x_qubit", "cavity": equal_cavity, "qubit": logical_plus},
        {"label": "equal_plus_i", "kind": "cavity_x_qubit", "cavity": equal_cavity, "qubit": logical_plus_i},
        {"label": "alternating_plus", "kind": "cavity_x_qubit", "cavity": alt_cavity, "qubit": logical_plus},
    ]
    for seed in (17, 29):
        rng = np.random.default_rng(seed)
        logical = rng.normal(size=2 * n_levels) + 1.0j * rng.normal(size=2 * n_levels)
        logical = logical / np.linalg.norm(logical)
        rows.append({"label": f"random_logical_seed{seed}", "kind": "logical_vector", "logical": logical})
    return tuple(rows)


def probe_state_metrics_from_full_operator(
    model: DispersiveTransmonCavityModel,
    levels: Sequence[int],
    *,
    full_operator: np.ndarray,
    ...
    rows: list[dict[str, Any]] = []
    for entry in probe_state_definitions(levels):
        if entry["kind"] == "cavity_x_qubit":
            amplitudes = np.kron(np.asarray(entry["cavity"], dtype=np.complex128), np.asarray(entry["qubit"], dtype=np.complex128))
            logical_vec = np.empty_like(amplitudes)
            logical_vec[0::2] = amplitudes[0::2]
            logical_vec[1::2] = amplitudes[1::2]
            psi0 = logical_state_qobj_from_vector(model, levels, logical_vec)
        else:
            psi0 = logical_state_qobj_from_vector(model, levels, np.asarray(entry["logical"], dtype=np.complex128))
        actual = qt.Qobj(np.asarray(full_operator, dtype=np.complex128) @ np.asarray(psi0.full(), dtype=np.complex128), dims=psi0.dims)
        ideal = apply_target_operator_to_state(model, levels, target_operator, psi0)
        fidelity = float(qt.fidelity(actual, ideal) ** 2)
        conditioned_rows: list[dict[str, float]] = []
        for level in levels:
            rho_q, population = conditioned_qubit_density_matrix(actual, int(model.n_cav), int(level))
        ...
        rows.append(
            {
                ...
                "conditioned_bloch_vectors": conditioned_rows,
            }
        )
    fidelities = np.asarray([float(row["state_fidelity"]) for row in rows], dtype=float)
    return {
        "probe_rows": rows,
