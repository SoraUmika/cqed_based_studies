...
def _builder_for_family(
    ...
    *,
    n_reference_segments: int | None = None,
) -> tuple[Callable[[np.ndarray, float], PulseDesign], tuple[tuple[float, float], ...]]:
    set_nulling_tail_kappa(cfg.kappa)
    n_reference_segments = cfg.reference_segments if n_reference_segments is None else int(n_reference_segments)
...
def _evaluator_for_regime(
    ...
    if regime == "multilevel":
        return partial(evaluate_multilevel_design, cfg=cfg)
    if regime == "hardware":
        return partial(evaluate_hardware_design, cfg=cfg)
    if regime == "nonlinear":
        return partial(evaluate_nonlinear_design, cfg=cfg)
    if regime == "rich":
        return partial(evaluate_rich_design, cfg=cfg)
...
def optimize_family(
    *,
        ...
        history.append(
            {
                ...
                "fidelity_eta": float(evaluation.metrics.fidelity_eta),
                "measurement_induced_transition": float(evaluation.metrics.measurement_induced_transition),
                "leakage": float(evaluation.metrics.leakage),
            }
        )
    ...
    if n_local > 0:
        for _, candidate_x, _, _ in scored_samples[: int(n_local)]:
            result = minimize(
                objective_fn,
                x0=candidate_x,
                method="Powell",
