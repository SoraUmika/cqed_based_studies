"""Shared utilities for the residual-Z cancellation waveform study."""
...
def gaussian_samples(count: int, sigma_fraction: float = DEFAULT_SIGMA_FRACTION) -> np.ndarray:
    ...
    norm_val = float(np.max(np.abs(samples)))
    if norm_val > 0.0:
        samples = samples / norm_val
    return samples
...
def conditioned_targets_from_blocks(blocks: Sequence[np.ndarray]) -> ConditionedQubitTargets:
    spec = {int(index): block_to_seed_angles(np.asarray(block, dtype=np.complex128)) for index, block in enumerate(blocks)}
...
def make_target_blocks(
    ...
    *,
    ...
    metadata: dict[str, Any] = {"target_family": str(target_family), "n_active": int(n_active)}
    if target_family == "C":
        euler_rows: list[tuple[float, float, float]] = []
        for level in range(int(n_active)):
            ...
            euler_rows.append((phi, theta, lam))
            blocks.append(su2_from_zyz(phi, theta, lam))
        metadata["smooth_euler_angles"] = euler_rows
    elif target_family == "D":
        if rng is None:
            raise ValueError("Random target family D requires an RNG.")
...
def restricted_blocks(restricted_operator: np.ndarray) -> tuple[np.ndarray, ...]:
    ...
    return tuple(op[2 * index : 2 * index + 2, 2 * index : 2 * index + 2] for index in range(op.shape[0] // 2))
...
def _simulation_session(model: DispersiveTransmonCavityModel, compiled: Any, *, frame: FrameSpec, drive_ops: dict[str, str]) -> Any:
...
def simulate_full_operator_on_logical_inputs(
    ...
    *,
    ...
    session = _simulation_session(model, compiled, frame=frame, drive_ops=drive_ops)
...
def simulate_state_for_sequence(
    model: DispersiveTransmonCavityModel,
    compiled: Any,
    *,
    frame: FrameSpec,
...
def state_validation_summary_for_sequence(
    model: DispersiveTransmonCavityModel,
    compiled: Any,
    *,
    frame: FrameSpec,
    ...
    for label, psi0 in (("cavity_superposition_ground", g_state), ("cavity_superposition_plus", plus_state)):
        actual = simulate_state_for_sequence(model, compiled, frame=frame, drive_ops=drive_ops, initial_state=psi0)
        ...
        rows.append({"label": label, "state_fidelity": float(fidelity), "conditioned_bloch_vectors": bloch_rows})
    return {"states": rows}
...
def tone_specs_from_targets(
    model: DispersiveTransmonCavityModel,
    targets: ConditionedQubitTargets,
    run_config: ConditionedMultitoneRunConfig,
) -> tuple[Any, ...]:
    return tuple(build_conditioned_multitone_tones(model, targets, run_config))
